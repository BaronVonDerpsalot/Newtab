...
export function freshDefaults() {
  return { theme:'dark', sessionsAvoided:0, startDate:todayStr(), exerciseDate:null,
    ...
    slips:0, slipsToday:0, lastInsight:null, lastInsightAt:null };
}

/* ── Local cache of last-good state ───────────────────
 * So the dashboard paints instantly and never blanks when the network is slow
 * or offline. Written on every state change; read on boot before the fetch. */
const CACHE_KEY = 'nt_state_cache';
export function cacheState(S) { try { localStorage.setItem(CACHE_KEY, JSON.stringify(S)); } catch {} }
export function readCachedState() { try { const r = localStorage.getItem(CACHE_KEY); return r ? JSON.parse(r) : null; } catch { return null; } }
...
export async function fetchState() {
  try {
    const{data,error}=await db.from('tab_state')
      .select('sessions_avoided,start_date,exercise_date,window_date,theme,good_stuff,slips,last_insight,last_insight_at')
      .eq('user_id',USER_ID).maybeSingle();
    if(error||!data) return{state:freshDefaults(),ok:false};
    let goodStuff=normalizeGoodStuff(data.good_stuff),sessionsToday=0,slipsToday=0;
    try {
      const eff=effectiveDateStr();
      const{data:row}=await db.from('daily_log')
        .select('values,sessions_today,slips_today').eq('user_id',USER_ID).eq('log_date',eff).maybeSingle();
      if(row){if(row.values&&typeof row.values==='object') goodStuff={date:eff,values:row.values}; sessionsToday=row.sessions_today??0; slipsToday=row.slips_today??0;}
    }catch{}
    return{state:{
...
export async function saveState(S) {
  ...
  try{const{error}=await db.from('tab_state').upsert({user_id:USER_ID,sessions_avoided:S.sessionsAvoided,start_date:S.startDate,exercise_date:S.exerciseDate,window_date:S.windowDate,theme:S.theme,good_stuff:S.goodStuff,slips:S.slips||0,last_insight:S.lastInsight,last_insight_at:S.lastInsightAt,updated_at:now},{onConflict:'user_id'});if(error)ok=false;}catch{ok=false;}
  try{const{error}=await db.from('daily_log').upsert({user_id:USER_ID,log_date:eff,values:S.goodStuff.values,exercise_done:S.exerciseDate===eff,window_open:windowOpenToday(),sessions_today:S.sessionsToday||0,slips_today:S.slipsToday||0,updated_at:now},{onConflict:'user_id,log_date'});if(error)ok=false;}catch{ok=false;}
  return ok;
}
export async function fetchHistory(days=30) {
  try{const{data}=await db.from('daily_log').select('log_date,values,exercise_done,window_open,sessions_today,slips_today').eq('user_id',USER_ID).order('log_date',{ascending:false}).limit(days);return data||[];}catch{return[];}
}
...
export async function buildStats(S) {
  ...
  return{today:effectiveDateStr(),days_active:daysActive(S.startDate),sessions_avoided_total:S.sessionsAvoided,sessions_avoided_today:S.sessionsToday,sessions_avoided_last7:asc.slice(-7).reduce((a,r)=>a+(r.sessions_today||0),0),slips_total:S.slips||0,slips_today:S.slipsToday||0,slips_last7:rows.slice(0,7).reduce((a,r)=>a+(r.slips_today||0),0),slips_last30:rows.reduce((a,r)=>a+(r.slips_today||0),0),exercise_done_last7:rows.slice(0,7).filter(r=>r.exercise_done).length,habits:habitSummary};
}
